...
def getWindows(img,kernel_dim=11):
    ...
    while True:
        ...
        R = 245.0
        GT_kernel_size = 11
        minus = int((GT_kernel_size - 1) / 2)
        plus = int((GT_kernel_size / 2) + 1)
    ...
    centers = np.array([list(each.values()) for each in center])
    windows = np.ones([centers.shape[0], 1, 64, 64]) * 0

    # X -> [0] Y -> [1]
    for i, w in enumerate(centers):
        # crop bounding box
        im = img[int(w[1] - (w[2]//2)):int(w[1] + (w[2]//2) + 1), int(w[0] - w[2]//2):int(w[0] + (w[2]//2) + 1)].copy()
        # remove background
        head_size = 250
        im[im > (w[3] + head_size)] = 0

        im = cv2.normalize(im.astype('float'), None, alpha=-1.0, beta=1.0, norm_type=cv2.NORM_MINMAX)
...
def prediction(filename,model,skel,imgnum,detection,localization,save=False,show=True):
    ...
    if kinectHead:
        cv2.rectangle(img, (int(x - range / 2), int(y - range / 2)), (int(x + range / 2), int(y + range / 2)),(0, 0, 255), 1)

    if show or save:
        for c in centers[pred[:, 0] > 0.95]:
            cv2.circle(img,(c[0],c[1]),1,(0,0,255),-1)
            cv2.rectangle(img, (int(c[0] - c[2] / 2), int(c[1] - c[2] / 2)), (int(c[0] + c[2] / 2), int(c[1] + c[2] / 2)), (0, 255, 0), 1)

    if np.max((pred[:, 0])) > 0.95:
        head = True
        if show or save:
            cv2.circle(img, (centers[max][0], centers[max][1]), 2, (255, 0, 0), -1)
            cv2.rectangle(img, (centers[max][0] - centers[max][2] // 2, centers[max][1] - centers[max][2] // 2),(centers[max][0] + centers[max][2] // 2, centers[max][1] + centers[max][2] // 2), (255, 0, 0), 1)
